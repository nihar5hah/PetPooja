...
export function WatchListTab({ cards }: { cards: StrategyCard[] }) {
  return (
    <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
      {cards.map((card, index) => (
        ...
        >
          <Card className="h-full border-rose-500/30 bg-card/95">
            <CardContent className="space-y-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h3 className="text-lg font-bold text-foreground">{card.item_name}</h3>
                  <span className="mt-2 inline-flex rounded-md bg-rose-500/10 px-2 py-1 text-[11px] font-semibold text-rose-300">
                    {card.category}
                  </span>
                </div>
                <div className="text-right">
                  <div className="text-4xl font-bold text-rose-300">{card.margin_pct.toFixed(1)}%</div>
              ...
              <div className="grid grid-cols-3 gap-2 text-center text-xs">
                <div className="rounded-xl border border-rose-500/20 bg-rose-500/10 p-3">
                  <div className="text-2xl font-bold text-rose-200">{card.units}</div>
                  <div className="text-[11px] uppercase tracking-wider text-muted-foreground">Units</div>
                </div>
                <div className="rounded-xl border border-rose-500/20 bg-rose-500/10 p-3">
                  <div className="text-2xl font-bold text-rose-200">{currency(card.revenue)}</div>
                  <div className="text-[11px] uppercase tracking-wider text-muted-foreground">Revenue</div>
                </div>
                <div className="rounded-xl border border-rose-500/20 bg-rose-500/10 p-3">
                  <div className="text-2xl font-bold text-rose-200">{currency(card.profit)}</div>
